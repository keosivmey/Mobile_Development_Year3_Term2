...
class RidesService {
  ...
  static List<Ride> _filterByDeparture(Location departure) {
    return availableRides.where((available)=>available.departture == departure).toList();
  }
  ...
  static List<Ride> _filterBySeatRequested(int requestedSeat) {
    return availableRides.where((available) => available.requestedSeat == requestedSeat).toList();
  }
